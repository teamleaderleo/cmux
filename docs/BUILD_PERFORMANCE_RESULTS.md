...
| Local rerun after host clear | reused DerivedData, 600s bound | 1 | timed out at 600.041s in the monolithic `A5001300` resource/helper shell phase | Swift compilation progressed beyond the prior cutoff; the host still had a concurrent unit build, so this is shared-host evidence and not a controlled warm result. Full receipt is retained with the investigation artifact. |
