...
final class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate, NSMenuItemValidation, NSMenuDelegate, CmuxConfigStoreReloadEnvironment {
    ...
    /// Compatibility name for the independently owned main-window model.
    typealias MainWindowContext = CmuxMainWindowContext
    ...
    private lazy var terminalCmdClickUITestCoordinator = TerminalCmdClickUITestCoordinator(
        dependencies: .init(
            environment: ProcessInfo.processInfo.environment,
            notificationCenter: .default,
            tabManager: { [weak self] in self?.tabManager },
            window: { [weak self] tabManager in
                guard let self,
                      let windowId = self.windowId(for: tabManager) else { return nil }
                return self.mainWindow(for: windowId)
            },
            managerToken: { [weak self] manager in
                self?.debugManagerToken(manager) ?? "nil"
            },
            sendText: { [weak self] text, workspace, beforeSend in
                self?.sendTextWhenReady(text, to: workspace, beforeSend: beforeSend)
            }
        )
    )
    ...
    func configure(
        ...
        terminalCmdClickUITestCoordinator.startIfNeeded()
