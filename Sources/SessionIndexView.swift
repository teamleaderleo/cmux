...
enum SessionTranscriptLoader {
    ...
    static func loadRecent(entry: SessionEntry) async -> [SessionTranscriptTurn] {
        let task = Task.detached(priority: .userInitiated) { () -> [SessionTranscriptTurn] in
            ...
            if url == nil, entry.agent == .codex {
                let home = ProcessInfo.processInfo.environment["CODEX_HOME"]
                    ?? NSHomeDirectory() + "/.codex"
                let rows = recentQuery(path: home + "/state_5.sqlite",
                    sql: "SELECT rollout_path FROM threads WHERE id = ? LIMIT 1", id: entry.sessionId)
                url = rows.first?.first.map { URL(fileURLWithPath: $0) }
            }
            if url == nil, entry.agent == .claude, let cwd = entry.cwd {
                ...
                // The UUID is a file component, never a caller-provided path.
                if UUID(uuidString: entry.sessionId) != nil {
                    url = URL(fileURLWithPath: home).appendingPathComponent("projects")
                        .appendingPathComponent(folder).appendingPathComponent(entry.sessionId + ".jsonl")
                }
            }
            ...
            for line in lines.reversed() {
                ...
                guard let turn = parseLineData(Data(line), agent: entry.agent,
                    usesGrokTranscriptLayout: entry.usesGrokTranscriptLayout, id: turns.count),
                    turn.role == .user || turn.role == .assistant else { continue }
                turns.append(SessionTranscriptTurn(id: turns.count, role: turn.role,
                    text: String(turn.text.suffix(4000))))
    ...
    private static func recentQuery(path: String, sql: String, id: String) -> [[String]] {
        ...
        guard sqlite3_bind_text(statement, 1, id, -1, transient) == SQLITE_OK else { return [] }
        var rows: [[String]] = []
        while !Task.isCancelled && rows.count < 64 && sqlite3_step(statement) == SQLITE_ROW {
            rows.append((0..<sqlite3_column_count(statement)).map { sqliteText(statement, $0) ?? "" })
        }
        return rows
    }

    private static func recentOpenCode(sessionID: String) -> [SessionTranscriptTurn] {
        let path = NSHomeDirectory() + "/.local/share/opencode/opencode.db"
        let rows = recentQuery(path: path, sql: """
            SELECT m.data, p.data FROM
              (SELECT id, data, time_created FROM message WHERE session_id = ?
               ORDER BY time_created DESC, id DESC LIMIT 12) m
            JOIN part p ON p.message_id = m.id
            WHERE length(p.data) <= 65536
            ORDER BY m.time_created DESC, m.id DESC, p.time_created DESC, p.id DESC LIMIT 64
            """, id: sessionID)
        var turns: [SessionTranscriptTurn] = []
        for row in rows {
