...
# Give the real hosted-Surface regression the same durable Launching state that
# production spawn_hosted expects. This helper is test-only.
...
'''
mux = replace_once(mux, mux_anchor, mux_helper + mux_anchor, "Mux launching test helper")
mux_path.write_text(mux)

# Replace the narrow classifier test with a real hosted Surface. The ACK is
# encoded onto the socket and consumed by the production reader thread; after
# it resolves, ordinary fire-and-forget input must still cross the same socket.
...
'''
surface = replace_once(surface, old_surface_test, new_surface_test, "real Surface reader ACK test")
surface_path.write_text(surface)

runtime = runtime_path.read_text()

# The attachment owns one independently clonable shutdown handle. Receipts
# share it without reacquiring or cloning through the frame-writer mutex.
...
""",
    """        reader: Option<UnixStream>,
        ...
        control_responses: Arc<ControlResponses>,
""",
    "HostAttachment shutdown handle",
)
runtime = replace_once(runtime, "        shutdown: UnixStream,\n", "        shutdown: Arc<UnixStream>,\n", "receipt shutdown type")

clone_block = r'''            let shutdown = {
...
'''
runtime = replace_once(runtime, clone_block, "", "per-request shutdown clone")
runtime = replace_once(
    runtime,
...
""",
    """                control_responses: self.control_responses.clone(),
                ...
                bytes: payload.len(),
""",
    "receipt shared shutdown construction",
)
...
""",
    """            reader: Some(reader),
            ...
            writer: Arc::new(Mutex::new(stream)),
""",
    "production attachment shutdown field",
)

# Test HostAttachments use an independent clone of the same socket.
for reader_shape in ("None", "Some(reader)"):
    old = f"""                reader: {reader_shape},
                writer: Arc::new(Mutex::new(client)),
"""
    new = f"""                reader: {reader_shape},
...
"""
    ...
    if count:
        ...
        print(f"updated HostAttachment test literals {reader_shape}: {count}")

# Make the timeout duration injectable for a focused lock-independence test.
...
'''
runtime = replace_once(runtime, old_wait, new_wait, "receipt testable timeout")

# Reuse a valid HostAttachment fixture from Surface tests without exposing any
# production API. It deliberately advertises input ACK support.
...
# Timeout must be able to abort the socket without waiting for the frame writer.
timeout_anchor = """        #[test]
...
# Prove the ACK is absent while either the PTY write or flush is still blocked.
...
'''
runtime = replace_once(runtime, old_order_test, new_order_test, "write+flush ACK ordering regression")

# Re-export the test-only fixture to surface.rs.
...
# Tighten what the documented certificate means, and make timeout poisoning an
# explicit availability choice instead of an accidental behavior.
