...
# Default: on, one store under the user's caches, size-limited.
out="$(args)"
[[ "$out" == *"COMPILATION_CACHE_ENABLE_CACHING=YES"* ]] || fail "caching is not enabled by default"
...
for off in 0 no false off; do
  [[ -z "$(args CMUX_COMPILATION_CACHE="$off")" ]] || fail "CMUX_COMPILATION_CACHE=$off still emits arguments"
done

# Overrides.
out="$(args CMUX_COMPILATION_CACHE_DIR=/Volumes/fast/cas CMUX_COMPILATION_CACHE_LIMIT_BYTES=1073741824)"
...
out="$(args "CMUX_COMPILATION_CACHE_DIR=/Volumes/my disk/cas")"
...
if args CMUX_COMPILATION_CACHE_DIR=relative/path >/dev/null 2>&1; then fail "relative store path accepted"; fi
if args CMUX_COMPILATION_CACHE_LIMIT_BYTES=lots >/dev/null 2>&1; then fail "non-numeric limit accepted"; fi
