...
fn enqueue_pending(
    ...
    // first record of the next bounded batch instead of making an over-capacity
    // ready batch.
...
mod tests {
    ...
    async fn busy_pool_backpressures_after_one_bounded_pending_batch() {
        ...
        assert!(!blocked.is_finished(), "an overflow busy record must wait for pool capacity");
        shared.cancellation.cancel();
